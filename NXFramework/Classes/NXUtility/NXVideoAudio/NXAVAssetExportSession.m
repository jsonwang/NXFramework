...
@interface NXAVAssetExportSession ()
...
@implementation NXAVAssetExportSession
{
    NSError *_error;
...
+ (id)exportSessionWithAsset:(AVAsset *)asset { return [NXAVAssetExportSession.alloc initWithAsset:asset]; }
...
- (void)exportAsynchronouslyWithCompletionHandler:(void (^)())handler
{
    ...
    if (videoTracks.count > 0)
    {
        [self.videoInput requestMediaDataWhenReadyOnQueue:self.inputQueue
                                               usingBlock:^{
                                                   if (![wself encodeReadySamplesFromOutput:wself.videoOutput
                                                                                    toInput:wself.videoInput])
                                                   {
                                                       @synchronized(wself)
                                                       {
                                                           videoCompleted = YES;
    ...
    {
        [self.audioInput requestMediaDataWhenReadyOnQueue:self.inputQueue
                                               usingBlock:^{
                                                   if (![wself encodeReadySamplesFromOutput:wself.audioOutput
                                                                                    toInput:wself.audioInput])
                                                   {
                                                       @synchronized(wself)
                                                       {
                                                           audioCompleted = YES;
        ...
        if (sampleBuffer)
        {
            ...
            if (!handled && self.videoOutput == output)
            {
                        ...
                        respondsToSelector:@selector(exportSession:renderFrame:withPresentationTime:toBuffer:)])
                {
                    ...
                    CVPixelBufferPoolCreatePixelBuffer(NULL, self.videoPixelBufferAdaptor.pixelBufferPool,
                                                       &renderBuffer);
                    ...
                    if (![self.videoPixelBufferAdaptor appendPixelBuffer:renderBuffer
                                                    withPresentationTime:lastSamplePresentationTime])
                    {
                        error = YES;
                    }
