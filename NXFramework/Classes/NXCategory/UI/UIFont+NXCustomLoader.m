 ...
 */
+ (BOOL)nx_deviceHasFullSupportForFontCollections
{
    return YES;
    //    return (CTFontManagerCreateFontDescriptorsFromURL != NULL);  // 10.6 or
    //    7.0
}
 ...
 */
+ (BOOL)nx_registerFromURL:(NSURL *)fontURL
{
    CFErrorRef error;
...
/**
 ...
 @return Registration result
 */
+ (BOOL)nx_registerFromCGFont:(CGFontRef)fontRef
{
    CFErrorRef error;
    BOOL registrationResult = YES;
...
+ (NSArray *)nx_registerFontFromURL:(NSURL *)fontURL
{
    // Dictionary creation
    ...
    @synchronized(appRegisteredCustomFonts)
    {
        ...
        {
            // Check features
            if ([UIFont nx_deviceHasFullSupportForFontCollections])
            {
                // Retrieve font descriptors from ttf, otf, ttc and otc files
                NSArray *fontDescriptors = (__bridge_transfer NSArray *)(CTFontManagerCreateFontDescriptorsFromURL(
                ...
                if (fontDescriptors)
                {
                    ...
                    for (NSDictionary *fontDescriptor in fontDescriptors)
                    {
                        ...
                        if (fontPSName)
                        {
                            ...
                            {
                                NSLog(@"Warning with font registration: Font '%@' already "
                                      @"registered",
                                      fontPSName);
                            }
                    ...
                    if ([fontPSNames count] > 0)
                    {
                        ...
                        if ([UIFont nx_registerFromURL:fontURL])
                        {
                    ...
                    {  // [fontPSNames count] <= 0
                        NSLog(@"Warning with font registration: All fonts in '%@' are "
                              @"already registered",
                              fontURL);
                    }
                }
                else
                {  // CTFontManagerCreateFontDescriptorsFromURL fail
                    NSLog(@"Error with font registration: File '%@' is not a Font", fontURL);
                    fontPSNames = nil;
                }
            }
            else
            {  // [UIFont deviceHasFullSupportForFontCollections] fail
                ...
                if (fontData)
                {
                    ...
                    if (loadedFont != NULL)
                    {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]])
                        {
                            ...
                            {
                                // If registration went ok
                                if ([UIFont nx_registerFromCGFont:loadedFont])
                                {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames forKey:fontURL];
                                }
                    ...
                    {  // CGFontCreateWithDataProvider fail
                        NSLog(@"Error with font registration: File '%@' is not a Font", fontURL);
                        fontPSNames = nil;
                    }
                ...
                {
                    NSLog(@"Error with font registration: URL '%@' cannot be read with "
                          @"error: %@",
                          fontURL, error);
                    fontPSNames = nil;
                }
...
+ (UIFont *)nx_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size
{
    ...
    NSArray *fontPSNames = [UIFont nx_registerFontFromURL:fontURL];
...
+ (UIFont *)nx_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension
{
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];

    return [UIFont nx_customFontWithURL:fontURL size:size];
}
