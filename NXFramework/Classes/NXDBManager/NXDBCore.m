    ...
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([rs next]) {
            if ([rs intForColumnIndex:0] == 0) {
                ...
                [self nx_createTable:db table_name:table_name fileds:fileds isAutoPrimaryKey:isAuto primaryKey:property_name_array objClass:objClass];
                [rs close];
                return ;
            }
        }
        ...
        for (NSString* property_name in fileds) {
            NSString *  property = [self nx_processReservedWord:property_name];
            if ([rs columnIndexForName:property] == -1)
            {
#ifndef NXDBLOGDISABLE
                NSLog(@"[%@表新增字段%@]", tableName, property);
#endif
                changes = [NSString stringWithFormat:@"%@|%@", changes, property];
        ...
        if (![changes isEqualToString:tableName]) {
            ...
            {
                sql = [NSString stringWithFormat:@"UPDATE NXDBVersion SET changes = '%@' WHERE version = '%@'", columnChanges, currentDBVersion];
...
- (void)nx_createTable:(FMDatabase *)db table_name:(NSString *)table_name fileds:(NSArray *)fileds isAutoPrimaryKey:(BOOL)isAuto primaryKey:(NSArray<NSString *> *)primaryKey objClass:(Class)objClass
{
    ...
    if (primaryKey.count > 1) {
        ...
        [self nx_createTableSingleKey:db table_name:table_name fileds:fileds primaryKey:primaryKey.firstObject objClass:objClass];
    }
}


- (void)nx_createTableAutoPrimaryKey:(FMDatabase*)db table_name:(NSString*)table_name fileds:(NSArray*)fileds objClass:(Class)objClass
{
    NSMutableString *sql = [[NSMutableString alloc] initWithFormat:@"CREATE TABLE %@ ( NXAUTOPRIMARYKEY integer primary key autoincrement, ", table_name ];
    ...
    for (NSString* property_name in fileds) {
        ...
        if (sqlType) {
            if ([dataObject respondsToSelector:@selector(nx_archiveProperty:)]) {
                ...
                {
                    property_value = [NSString stringWithFormat:@"'%@',", [[NSString alloc] initWithData:result encoding:NSUTF8StringEncoding]];
                }
                else
                {
...
- (NSString *)nx_formatDeleteSQLWithObjc:(id<NXDBObjectProtocol>)data_obj withTableName:(NSString*)tableName
{
    ...
    if (data_obj) {
        ...
        if (property_name_array.count > 1) {
            ...
            condition = [self nx_formatSingleConditionSQLWithObjc:data_obj property_name:property_name_array.firstObject];
        }
