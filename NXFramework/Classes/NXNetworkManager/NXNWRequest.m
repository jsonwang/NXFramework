...
- (instancetype)init { return [self initWithUrl:@""]; }
...
/**
 获取最终生效的请求url, 当baseUrl 和 url同时设置时，根据 ingoreBaseUrl
 字段返回url
...
/**
 取消当前请求
 */
- (void)cancelRequset { [[NXNWCerter shareInstanced] cancleRequest:self.identifier]; }
/**
 暂停请求
 */
- (void)pauseRequest { [[NXNWCerter shareInstanced] pasueRequest:self.identifier]; }
/**
 恢复请求
 */
- (void)resumeRequst { [[NXNWCerter shareInstanced] resumeRequest:self.identifier request:self]; }
/**
 准备发送请求
 */
- (void)start { [self startWithSucces:self.succesHandlerBlock failure:self.failureHandlerBlock]; }
/**
 准备发送请求
...
{
    NXUploadFormData *formData =
        [NXUploadFormData formDataWithName:name fileName:fileName mimeType:mimeType fileData:fileData];
    [self.uploadFileArray addObject:formData];
}
...
{
    NXUploadFormData *formData =
        [NXUploadFormData formDataWithName:name fileName:fileName mimeType:mimeType fileURL:fileURL];
    [self.uploadFileArray addObject:formData];
}

- (BOOL)hasRepeatRequest { return [[NXNWCerter shareInstanced] hasRepeatRequest:self]; }
@end
...
- (void)startRequset { [self startWithSuccess:self.successBlock failure:self.failureBlock]; }
...
- (void)startWithRequest:(NXAddBatchRequestBlock)bactchRequestBlock
...
{
    NSAssert(buildBlock, @"构建链式请求的 buildBlock "
                         @"不能为空！！！");
    self.buildBlock = nil;
...
- (void)startRequest { [self startWithSucces:self.succesBlock failure:self.failureBlock]; }
...
- (void)startWithNodeBuild:(NXChainNodeBuildBlock)nodeBuildBlock
