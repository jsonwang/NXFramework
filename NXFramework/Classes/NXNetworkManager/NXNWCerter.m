...
- (NXNWBridge *)brdge { return [NXNWBridge shareInstaced]; }
...
{
    return [self sendRequset:requset
                    progress:progressBlock
                      succes:requset.succesHandlerBlock
                     failure:requset.failureHandlerBlock];
}
...
- (NSString *)nx_sendRequest:(NXNWRequest *)request
{
    if (request.config.consoleLog)
    {
        if (request.requstType == NXNWRequestTypeDownload)
        {
            NSLog(@"\n============ [NXNWRequest Info] ============\nrequest download "
                  @"url: %@\nrequest save path: %@ \nrequest headers: \n%@ \nrequest "
                  @"parameters: \n%@ \n==========================================\n",
                  request.fullUrl, request.fileUrl, request.headers.containerConfigDic,
                  request.params.containerConfigDic);
        }
        else
        {
            NSLog(@"\n============ [NXNWRequest Info] ============\nrequest url: %@ "
    ...
    [self.brdge sendWithRequst:request
...
- (void)nx_processBatch:(NXNWRequest *)request
           batchRequest:(NXBatchRequest *)bRequest
            responseObj:(id)response
                  error:(NSError *)error
{
    [self.lock lock];
...
- (void)nx_processChainRequest:(NXNWRequest *)request
                  chainRequest:(NXChainRequest *)chainRequest
                   responseObj:(id)response
                         error:(NSError *)error
{
    [self.lock lock];
...
- (void)nx_runNextChainRequest:(NXChainRequest *)chainRequest
{
    ...
    {
        __weak typeof(self) weakSelf = self;
        [self sendRequset:request
            succes:^(id responseObject, NXNWRequest *rq) {
                [weakSelf nx_processChainRequest:request
...
- (void)pasueRequest:(NSString *)identifier { [[NXNWBridge shareInstaced] pauseRequest:identifier]; }
- (void)cancleRequest:(NSString *)identifier { [[NXNWBridge shareInstaced] cancleRequst:identifier]; }
- (void)resumeRequest:(NSString *)identifier request:(NXNWRequest *)request
{
    [[NXNWBridge shareInstaced] resumeRequest:identifier];
}
- (id)getRequest:(NSString *)identifier { return [[NXNWBridge shareInstaced] getRequestByIdentifier:identifier]; }
- (void)addSSLPinningURL:(NSString *)url { [self.brdge addSSLPinningURL:url]; }
- (void)addSSLPinningCert:(NSData *)cert { [self.brdge addSSLPinningCert:cert]; }
...
- (BOOL)hasRepeatRequest:(NXNWRequest *)request { return [self.brdge hasRepeatRequest:request]; }
@end
