...
- (void)bindingRequest:(NXNWRequest *)request {
    objc_setAssociatedObject(self, (__bridge CFStringRef)NXNWRequestBindingKey, request, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)nx_parseResponse:(NSURLResponse *)response responseObj:(id)responseObj error:(NSError *)error
                 request:(NXNWRequest *)request completionHandler:(NXCompleteBlcok) completeHandler{
...
- (void)nx_bindTask:(NXNWRequest *)requset dataTaskIdentifier:(NSURLSessionDataTask *)dataTask sessionManager:(AFHTTPSessionManager *) sessionManager{
